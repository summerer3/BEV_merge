...
parser.add_argument('-path', '--INPUT_PATH', default='../data/intrinsic/front/', type=str, help='Input Video/Image Path')
parser.add_argument('-video', '--VIDEO_FILE', default='video.mp4', type=str, help='Input Video File Name (eg.: video.mp4)')
parser.add_argument('-image', '--IMAGE_FILE', default='', type=str, help='Input Image File Name Prefix (eg.: img_raw)')
...
parser.add_argument('-fh','--FRAME_HEIGHT', default=800, type=int, help='Camera Frame Height')
...
parser.add_argument('-size','--SQUARE_SIZE', default=40, type=int, help='Chess Board Square Size (mm)')
...
def main(IN_PUT):
    print('='*20+IN_PUT+'='*20)
    args.INPUT_PATH = 'data/intrinsic/'+IN_PUT
    ...
    # if IN_PUT=='front':
    #     cx,cy,fx,fy=641.446667 ,399.165321 ,314.047920 ,314.047920
    #     dist=[0.119481, -0.016275, 0.000604, -0.000428]
    # elif IN_PUT=='left':
    #     cx,cy,fx,fy=633.215083, 407.438519, 314.047920, 314.047920
    #     dist=[0.119481, -0.016275 ,0.000604 ,-0.000428]
    # elif IN_PUT=='right':
    #     cx,cy,fx,fy=641.371567 ,397.216477 ,314.047920 ,314.047920
    #     dist=[0.119481, -0.016275, 0.000604, -0.000428]
    # elif IN_PUT=='back':
    #     cx,cy,fx,fy=643.032028, 392.513965 ,314.047920 ,314.047920
    #     dist=[0.119481, -0.016275, 0.000604, -0.000428]
    
    # result.camera_mat=[[fx,0,cx],
    #         [0,fy,cy],
    #         [0,0,1]]
    # result.dist_coeff=dist
    
    print("Calibration Complete")
    print("Camera Matrix is : \n{}".format(result.camera_mat)) 
    print("Distortion Coefficient is : \n{}".format(result.dist_coeff))
    print("Reprojection Error is : \n{}".format(np.mean(result.reproj_err))) 
    np.save(args.INPUT_PATH+'/camera_{}_K.npy'.format(IN_PUT),result.camera_mat)
    np.save(args.INPUT_PATH+'/camera_{}_D.npy'.format(IN_PUT),result.dist_coeff)
